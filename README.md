# 🪙 Coin Alert v5.44 — Upbit KRW 자동매매 (평균회귀)
...
| 거래대금 | ≥ 30억원 (24h) | 유동성 확보 |
| 재매수 차단 | 매도가 대비 -3% 이상 하락 | 같은 가격대 재진입 방지 |
...
| **STOP_LOSS** | DCA 미발동: PnL ≤ **-5%** | 전량매도 | 진입가 기준 (v5.44) |
| | DCA 발동 후: PnL ≤ **-4%** | 전량매도 | 평균단가 기준 (v5.44) |
...
```
             TP1(분할)  RSI매도   SL(DCA전/후)
BEAR         +3%       RSI ≥ 60   -5% / -4%    빨리 수익 확정
MILD_BEAR    +2.5%     RSI ≥ 65   -5% / -4%
SIDEWAYS     +3%       RSI ≥ 70   -5% / -4%    표준
MILD_BULL    +3%       RSI ≥ 75   -5% / -4%
BULL         +4%       RSI ≥ 80   -5% / -4%    추세를 충분히 탄다
```
