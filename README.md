# 🪙 Coin Alert v5.48 — Upbit KRW 자동매매 (평균회귀)
...
2차 매수: 진입가 대비 -7% 하락 시 나머지 40% 추가 매수 (DCA_DROP_PCT = 7%)
→ 평균단가 개선 + DCA-SL 레이스 해소 (v5.48: 5→7%)
```
...
| **PARTIAL_SL1** | PnL ≤ **-4%** | **50% 분할매도** | 1단계 손절, 반등 대기 |
| **STOP_LOSS** (SL2) | PnL ≤ **-6%** | **나머지 전량매도** | 2단계 손절, 구조적 하락 |

> **분할 손절 근거 (v5.48)**: 수익은 분할매도(TP1 50%)하면서 손절은 전량매도하는 비대칭 해소. 백테스트: SL 평균 손실 -5.83% → -2.68% (54% 감소). DCA 후에는 1단계 SL이 DCA평단 -4% 기준.
...
```
             TP1(분할)  RSI매도   SL(분할)              진입점수
BEAR         +3%       RSI ≥ 60   SL1 -4%→50%, SL2 -6%  ≥ 7점    빨리 수익 확정
MILD_BEAR    +2.5%     RSI ≥ 65   SL1 -4%→50%, SL2 -6%  ≥ 7점
SIDEWAYS     +3%       RSI ≥ 70   SL1 -4%→50%, SL2 -6%  ≥ 6점    표준
MILD_BULL    +3%       RSI ≥ 75   SL1 -4%→50%, SL2 -6%  ≥ 6점
BULL         +4%       RSI ≥ 80   SL1 -4%→50%, SL2 -6%  ≥ 5점    추세를 충분히 탄다
```
