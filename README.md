...
- **최소 보유 3시간**: 진입 직후 노이즈에 의한 스탑/신호 매도 방지
  - 트레일링 스탑 + 앙상블 신호 매도 모두 적용 (STRONG_CLOSE 긴급 청산은 예외)
  - 레짐 전환 노이즈로 인한 일괄 조기 매도 방지
- **매도 후 즉시 재매수**: 같은 사이클에서 CLOSE→BUY 자금 재배치 가능 (자본 회전 버그 수정)
- 분석 결과: 0~3시간 보유 거래 = 승률 0% → 스탑/신호 유예로 개선
...
## 리스크 관리
- **최소 보유시간**: 3시간 이내 매도 차단 (트레일링 스탑 + 신호 매도 모두, STRONG_CLOSE 예외)
- **트레일링 스탑**: ATR × 2.0 (빠른 손절 + 쿨다운 보호)
- **손절 쿨다운**: 손실 매도 후 같은 종목 24시간 재진입 금지
