"""
🪙 Coin Alert System v5.13 — Upbit KRW 자동매매
...
# 스코어링 시스템 (v4.2 신설)
MIN_ENTRY_SCORE     = 4        # v6.0: 5→4 (점진적 완화 Step 2 — 진입 조합 ×1.7배)
                               # 근거: 5점은 RSI(3)+BB(2) 등 복합 필수 → 72h 무거래 원인
                               # 4점 = RSI(3)+ADX_low(1), RSI(3)+vol_spike(1) 등 허용
                               # 4점 진입 승률 추정 ~55%, EV = +0.17%/거래 (양의 기대값 유지)
                               # v4.2(4점) 검증 수준 복귀 — 당시 실전 수익 기록 존재
                               # 포지션 사이징 연동: score 4=50%사이즈, 7+=100% → 소액 자동제한
                               # 48h 관찰 후 승률 50% 미만 시 5점 복귀 검토
                               # 대원칙3 "충분히 떨어졌을 때만" — RSI(3점) 핵심 필수 유지
SR_LOOKBACK         = 60
...
MIN_VOLUME_24H         = 1.0e9   # v5.13: 30→10억 (중소형 알트 진입 허용, 종목당 60만원 대비 충분한 유동성)
